...
また、「常に最新の Tag を取得する」方法をとることで他の問題も回避できる。具体的な問題は以下のとおり。

aws_rds_cluster の cluster_identifier を変更するとエンドポイントも変更される。  
タスク定義では aws_rds_cluster のエンドポイントを環境変数として受け取るようにしている。  
しかし、エンドポイントの変更に伴い、aws_ecs_task_definition 自体は変更されたのだが、aws_ecs_service のタスク定義は変更されなかった。  
前述の通り、aws_ecs_service は作成した aws_ecs_task_definition を参照するのではなく、最新のタスク定義を取得して参照している。  
その辺りを上手く制御できなかったことが原因だと思うが、作成した aws_ecs_task_definition を直接参照するようにしていれば起きなかったはず。

「常に最新の Tag を取得する」ようにしていれば、差分が起きないように最新のタスク定義を取得して参照する意味もなくなるので、この問題も起きないと思う。

### KMS

基本的に規模が大きく、成熟した PJ であれば、マネージドキーではなく、カスタマー管理キー（CMK）を利用すべきだと思う。  
かつ、サービスごとに CMK を作成すべきだとも思う。  
ただ、スタートアップなどの少人数 PJ であれば、緩いアクセス制限が大きな問題になることはない気がするので、コストの問題からマネージドキーで問題ないと思う。

このリポジトリでは RDS は CMK を使い、SSM パラメータストアはマネージドキーを使ってみた。
